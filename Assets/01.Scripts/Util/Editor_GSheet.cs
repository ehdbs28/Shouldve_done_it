...
public class Editor_GSheet : EditorWindow
{
    ...
    public string validatorSheetName;
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        urlid = EditorGUILayout.TextField("URL ID", urlid, GUILayout.Width(540f));
        EditorGUILayout.EndHorizontal();
        EditorGUILayout.BeginHorizontal();
        gid = EditorGUILayout.TextField("GID", gid, GUILayout.Width(540f));
        EditorGUILayout.EndHorizontal();

        if (GUILayout.Button("Add"))
        {
            if (string.IsNullOrEmpty(sheetName) || string.IsNullOrEmpty(urlid) || string.IsNullOrEmpty(gid)) return;

            domain.Add(sheetName, urlid, gid);
            domain.Save();
        }
    ...
    {
        string soPath = Path.Combine(Application.dataPath,"Resources/Json/TranslationSO");
        ...
        for (int i = 0; i < domain.Count; ++i)
        {
            ...
            FileSave(pdPath, domain.SheetNameList[i] + ".json", PaserJson(csv));
        }
    }
    ...
    private void ButtonSaveCs()
    {
        ...
        string classText = LoadText(Application.dataPath + "/" + "GB/EditorText/ExportClass.txt");
        string dataManagerText = LoadText(Application.dataPath + "/" + "GB/EditorText/ExportGameDataManager.txt");
...
public class GSheetDomain
{
    ...
    public List<string> UrlIDList = new List<string>();
    ...
    public void SetJson(string json)
    {
        ...
        UrlIDList = data.UrlIDList;
        GidList = data.GidList;
        SheetNameList = data.SheetNameList;
    }

    public void Add(string sheetName, string UrlID, string Gid)
    {
    ...
    public void Remove(int index)
    {
        ...
        UrlIDList.RemoveAt(index);
        GidList.RemoveAt(index);
        Save();
    }
    ...
    public string GetURL(int index)
    {
        ...
        string url = URL_SHEET.Replace("$URL_ID$", UrlIDList[index]).Replace("$GID$", GidList[index]);
    ...
    public string ToJson()
    {
        ...
        gsheet.UrlIDList = UrlIDList;
        gsheet.GidList = GidList;
