...
namespace Episode.Room
{
    public class FemaleCharacter : Character
    {
        ...
        {
            ShowPhoneSpeechBubble.Show(SpeechBubble.SpeechBubbleType.Text, true, "너 뭐하냐...?");

            yield return new WaitForSeconds(2f);

            Room.SetResult("들킨 잘못은 변명이 아닌 대가를 요구한다.", "Rowan Blake", false);
        }
        ...
        public void OnStandUp()
        {
            ...
            //seq.AppendCallback(() =>
            //{
            //    Room.SetResult("??? ???? ???? ????????, ??? ?? ?Ŀ??? ?????? ???? ???.", "Rowan Blake", false);
            //});
